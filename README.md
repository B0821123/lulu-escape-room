...
由「守書人」貫穿全程的連貫故事：一座圖書館裡，屬於你們的《告白之書》被封住、書頁散成三段記憶與數個頁面印記，Lulu 要把它們一一尋回。

| 線 | 名稱（象徵） | 謎題 |
|---|---|---|
| 主線 | 告白之書 · 此刻的星圖 | 咒語密碼筒(`LULU PIGGY`) → 邏輯推理印記(`493`) → 圖形矩陣(XOR) → 七位頁角合鑰(`1455436`) |
| 支線一 | 告白之書 · 第一頁的日期 | 日期門(`20250214`) |
| 支線二 | 告白之書 · 心意的回聲 | 摩斯光訊(`BOBO`) → 光徑迷宮 → 兔兔食庫拼字鎖(`RABBIT POOP`) |
...
- `answers.rabbitFood`：兔兔食庫拼字鎖答案（散落字磚拼回的英文片語），目前是 `RABBIT POOP`
